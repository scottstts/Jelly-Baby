...
type SoccerSampleKind=SoccerEvent|'run'|'land';

export function soccerSample(kind:SoccerSampleKind,sampleRate:number) {
  const duration=kind==='run'?.5:kind==='land'?.24:kind==='goal'?.75:.20,data=new Float32Array(Math.ceil(duration*sampleRate));
  ...
  for(let i=0;i<data.length;i++) {
    ...
    if(kind==='run') {
      ...
      // Keep the landing in the same dry grass family as the run loop, with a
      // short soft low pulse instead of the hard-surface membrane modes.
      const grass=noise-low,foot=Math.exp(-t*22);
      data[i]=grass*(.028+.075*foot)+low*.052*foot+.045*Math.sin(2*Math.PI*86*t)*Math.exp(-t*18);
    } else if(kind==='goal') {
...
export class SoccerAudio {
  ...
  private buffer(kind:SoccerSampleKind) {
  ...
  motion(speed:number,distance:number,pan:number) {
    ...
    const amount=Math.min(1,speed/.44);this.gain.gain.setTargetAtTime(speed>.012?.16*Math.sqrt(amount)/(1+(distance/.85)**2):0,ctx.currentTime,.035);
  ...
  landing(strength:number,distance:number,pan:number) {
    const ctx=this.context;if(ctx.state!=='running'||this.voices.size>=8||ctx.currentTime-this.lastEvent<.055)return;
    this.lastEvent=ctx.currentTime;const source=ctx.createBufferSource(),gain=ctx.createGain(),panner=ctx.createStereoPanner();source.buffer=this.buffer('land');
    gain.gain.value=.82*Math.min(1,strength)/(1+(distance/1.7)**2);panner.pan.value=pan;
