...
export class JellySkates {
  ...
  constructor(keepParts=false) {
    ...
    for(const side of [-1,1]) {
      const frame=new Group();frame.name=`cradle-${side}`;frame.userData.keepParts=keepParts;frame.position.x=side*SKATE.halfWidth;this.group.add(frame);
      ...
      for(let i=0;i<3;i++) {
        const wheel=new Group();wheel.name=`wheel-${side}-${i}`;wheel.position.set(side*SKATE.halfWidth,SKATE.wheelRadius,(i-1)*.011);this.group.add(wheel);this.wheels.push(wheel);
  ...
  update(position:Vector3,yaw:number,distance:number){this.group.position.copy(position);this.group.rotation.y=yaw;for(const wheel of this.wheels)wheel.rotation.x=distance/SKATE.wheelRadius;}
